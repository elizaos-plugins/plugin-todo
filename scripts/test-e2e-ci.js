...
const MAX_RETRIES = parseInt(process.env.TEST_MAX_RETRIES, 10) || 60;
const RETRY_DELAY = parseInt(process.env.TEST_RETRY_DELAY, 10) || 1000;
    ...
    serverProcess.stdout.on("data", (data) => {
      ...
      if (VERBOSE) {
        process.stdout.write(
          `${colors.magenta}[Server]${colors.reset} ${text}`,
        );
      }
    });
    ...
    serverProcess.stderr.on("data", (data) => {
      ...
      if (VERBOSE || text.toLowerCase().includes("error")) {
        process.stderr.write(
          `${colors.red}[Server Error]${colors.reset} ${text}`,
        );
      }
    });
