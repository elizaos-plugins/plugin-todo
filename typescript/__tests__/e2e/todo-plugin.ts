import type { Content, IAgentRuntime, ServiceType } from "@elizaos/core";
  ...
  tests: [
    ...
    },
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        const _mockCallback = async (response: Content) => {
          if (response.text?.includes("finish my report")) {
    ...
    },
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        // Create test data
        const testRoomId = `test-room-${Date.now()}` as UUID;
        const _testEntityId = `test-entity-${Date.now()}` as UUID;
        const testWorldId = `test-world-${Date.now()}` as UUID;
